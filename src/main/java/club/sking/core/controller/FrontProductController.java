...
import club.sking.common.web.ResponseUtils;
import club.sking.common.web.session.SessionProvider;
import club.sking.core.bean.product.*;
...
import club.sking.core.service.product.*;
...
public class FrontProductController {
	...
	public void list_index(HttpServletRequest request, HttpServletResponse response){
		...
		List<Product> recentlyProducts = new ArrayList<Product>();

		//为了在页面显示到不同的栏目,重新分组
		Iterator<Product> iterator = productList.iterator();
		while (iterator.hasNext()){
			Product next = iterator.next();
			if (next.getIsNew()!= null && next.getIsNew().equals(1)){
				// 新品
				goodProducts.add(next);
			}else if (next.getIsHot() != null && next.getIsHot().equals(1)){
				// 热销
				hotProducts.add(next);
			}else if (next.getIsCommend() != null && next.getIsCommend().equals(1)){
				// 推荐
				recentlyProducts.add(next);
			}
		}

		JSONObject jsonObject = new JSONObject();
		jsonObject.put("goodProducts",goodProducts);
		jsonObject.put("hotProducts",hotProducts);
		jsonObject.put("recentlyProducts",recentlyProducts);
